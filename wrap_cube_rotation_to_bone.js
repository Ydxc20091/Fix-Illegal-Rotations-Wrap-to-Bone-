...
Plugin.register('wrap_cube_rotation_to_bone', {
  ...
  onload() {
    ...
    // ======== Action 6: Group bones with same rotation ========
    const action_group_same_rotation = new Action('group_same_rotation_bones', {
      name: 'Group Bones with Same Rotation',
      ...
      click: () => {
        ...
        if (!bones.length) {
          Blockbench.showQuickMessage('No bones found in the project.');
          return;
        }
        ...
        if (!groupsToProcess.length) {
          Blockbench.showQuickMessage('No bones with same rotation found (excluding zero rotation).');
          return;
        }
        ...
        groupsToProcess.forEach(([rotKey, bones]) => {
          ...
          // 创建新的父骨骼
          const parentBoneName = makeUnique(`RotationGroup_${rotation[0] || 0}_${rotation[1] || 0}_${rotation[2] || 0}`);
        ...
        Canvas.updateAll();
        Undo.finishEdit(`Grouped ${boneCount} bones into ${groupCount} rotation groups`);
        Blockbench.showQuickMessage(`Done: Grouped ${boneCount} bones with same rotation into ${groupCount} parent bones. Cube positions preserved.`);
      }
    });
